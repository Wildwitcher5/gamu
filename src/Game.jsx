...
export default function App(){
  ...
  const alexSpeak=async(eventType,g)=>{
    ...
    const hints={
      trade_offer:`Предложи партнёру обменяться картой — одна короткая реплика, каждый раз разными словами. Пиши как живой игрок в чате: не «предлагаю обмен», а что-то вроде «поменяемся?» / «у меня лишнее» / «глянь что есть».`,
      trade_accepted:`Партнёр принял обмен. Одна короткая реакция: «ок», «збс», «давай», «договорились», «норм» — каждый раз по-разному.`,
      trade_declined:`Партнёр отказался. Одна спокойная фраза: «ладно», «ок», «понял», «ну и норм».`,
      low_hp:`HP очень низкое (твоё: ${allyHp}/100, партнёра: ${youHp}/100). ${tacticalNote||""} Напиши практично — попроси помощь или укажи на ситуацию: «есть хил?», «плохо становится», «нужна помощь», «похилишь?». Одна фраза.`,
      enemy_low_hp:`Враги почти добиты (${ctx.enemies}). Одна короткая реплика: «добиваем», «ещё чуть», «го», «почти» и т.п.`,
      took_heavy_hit:`Партнёр получил сильный удар, у него ${youHp}HP. ${tacticalNote||""} Коротко и по-делу: «держись», «плохо», «похилить?», «нужна помощь?». Одна фраза, каждый раз другими словами.`,
      victory:`Победили. Одна фраза — «gg», «сделали», «уф», «норм сыграли», «молодцы» и т.п. В своём стиле.`,
      defeat:`Проиграли. Одна фраза без нытья: «бывает», «ну и ладно», «gg», «не повезло».`,
      joint_combo:`Только что нанесли совместный удар. Короткая реакция — «gg», «вот это да», «норм», «да» или просто молчание [SKIP].`,
      ally_down:`Ты упал в бою (HP=0). Напиши партнёру что пал и нужно Возрождение — коротко, своими словами.`,
